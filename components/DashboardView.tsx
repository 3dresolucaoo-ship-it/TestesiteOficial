...
import { Sparkline, RevenueLineChart } from '@/components/FinanceCharts'
...
import {
  ...
  Package, Layers, Activity, ChevronDown,
} from 'lucide-react'
      ...
      <Section title="Visão Financeira" icon={TrendingUp} iconColor="text-[#10b981]" href="/finance">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          <StatCard
            ...
            sub={`${state.transactions.filter(t => t.type === 'income').length} transações`}
            icon={TrendingUp}
            accentColor="#10b981"
          ...
          <StatCard
            ...
            sub={stats.profit >= 0 ? 'resultado positivo' : 'resultado negativo'}
            icon={DollarSign}
            accentColor={stats.profit >= 0 ? '#10b981' : '#ef4444'}
            sparkData={profitSparkData}
            sparkColor={stats.profit >= 0 ? '#10b981' : '#ef4444'}
          />
          <StatCard
            label="Pedidos Ativos"
            value={String(stats.activeOrders)}
            sub={`${state.orders.length} total`}
            icon={ShoppingCart}
            accentColor="#3b82f6"
          />
        </div>
        {monthly.length >= 2 && (
          <Panel>
            <p className="text-[#555566] text-[11px] font-semibold uppercase tracking-wider mb-4">
              Receita &amp; Lucro — Últimos {monthly.length} meses
            </p>
      ...
      <Section title="Operações" icon={Printer} iconColor="text-[#f59e0b]" href="/production">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            sub={`${state.production.filter(p => p.status === 'waiting').length} aguardando`}
            icon={Printer}
            accentColor="#f59e0b"
          />
          <StatCard
            ...
            sub={`potencial ${fmt(inventoryProfitPotential)}`}
            icon={Package}
            accentColor="#a78bfa"
          />
          <StatCard
          ...
          <StatCard
            ...
            sub={`${stats.contentPosted} postados`}
            icon={Eye}
            accentColor="#a78bfa"
          />
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-4">
          <Panel className="lg:col-span-3">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/projects" className="text-[#444455] hover:text-[#f0f0f5] transition-colors">
                    ...
                    <Link key={p.id} href={`/projects/${p.id}`} className="flex items-center gap-4 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[#f0f0f5] text-xs font-medium group-hover:text-white transition-colors truncate">
                            {p.name}
                          </span>
                          <div className="flex items-center gap-2 ml-2 shrink-0">
                            <span className={`text-[11px] font-medium ${margin >= 0 ? 'text-[#10b981]' : 'text-[#ef4444]'}`}>
                              {margin.toFixed(0)}%
                            </span>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <span className={`text-[11px] font-medium ${p.profit >= 0 ? 'text-[#10b981]' : 'text-[#ef4444]'}`}>
                            Lucro {fmt(p.profit)}
                          </span>
                        </div>
                      </div>
          ...
          <Panel className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/finance" className="text-[#444455] hover:text-[#f0f0f5] transition-colors">
                <ArrowUpRight size={14} />
              </Link>
            </div>
            {recentTx.length === 0 ? (
              ...
              <div className="space-y-px">
                {recentTx.map(t => {
                  ...
                  return (
                    <div
                      key={t.id}
                      className="flex items-center gap-3 py-2.5 border-b border-[rgba(255,255,255,0.04)] last:border-0 hover:bg-[rgba(255,255,255,0.02)] rounded-lg px-2 -mx-2 transition-colors"
                    >
                      <div
          ...
          <Panel glow="#7c3aed">
            ...
            {topInventoryItems.length > 0 && (
              <div className="space-y-2.5">
                <p className="text-[#444455] text-[11px] font-semibold uppercase tracking-widest mb-3">
                    ...
                    <div key={item.id}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-[#444455] text-[10px] whitespace-nowrap">
                            {item.quantity} {item.unit}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 shrink-0 ml-2">
                          <span className={`text-[11px] font-medium ${item.profitPot >= 0 ? 'text-[#10b981]' : 'text-[#ef4444]'}`}>
                            +{fmtFull(item.profitPot)}
                          </span>
                          <span className="text-[#f0f0f5] text-xs font-bold tabular-nums">
                            {fmt(item.totalValue)}
                          </span>
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: 'Receita Produtos', value: fmt(prodIntel.totalRevenue),        color: '#10b981', icon: TrendingUp },
                {
                  ...
                  value: fmt(prodIntel.totalProfit),
                  color: prodIntel.totalProfit >= 0 ? '#10b981' : '#ef4444',
                  icon:  DollarSign,
                },
                {
                  label: 'Margem Geral',
                  value: `${prodIntel.overallMargin.toFixed(1)}%`,
                  color: prodIntel.overallMargin >= 40 ? '#a78bfa' : prodIntel.overallMargin >= 20 ? '#f59e0b' : '#ef4444',
                  icon:  Flame,
                },
                ...
                <div
                  ...
                  style={{ background: `${color}0d`, border: `1px solid ${color}22`, boxShadow: `0 4px 20px rgba(0,0,0,0.3)` }}
                >
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {prodIntel.bestProduct && (
                  <div
                    ...
                    style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.20)', boxShadow: '0 0 20px rgba(16,185,129,0.08)' }}
                  >
                    <div>
                      ...
                      <p className="text-[#555566] text-xs mt-0.5">
                        {prodIntel.bestProduct.orderCount} pedido(s) · {fmt(prodIntel.bestProduct.totalProfit)} lucro
                      </p>
                    </div>
                    <span className="text-[#10b981] text-xl font-bold shrink-0" style={{ textShadow: '0 0 20px rgba(16,185,129,0.5)' }}>
                      {prodIntel.bestProduct.avgMargin.toFixed(0)}%
                    </span>
                  </div>
                )}
                {prodIntel.worstProduct && (
                  <div
                    className="rounded-xl px-4 py-3 flex items-start justify-between"
                    style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.20)', boxShadow: '0 0 20px rgba(239,68,68,0.08)' }}
                  >
                    <div>
                      <p className="text-[#444455] text-[10px] font-semibold uppercase tracking-wider mb-1">⚠️ Menos Rentável</p>
                      <p className="text-[#f0f0f5] text-sm font-semibold">{prodIntel.worstProduct.product.name}</p>
                      <p className="text-[#555566] text-xs mt-0.5">
                        {prodIntel.worstProduct.orderCount} pedido(s) · margem {prodIntel.worstProduct.avgMargin.toFixed(1)}%
                      </p>
                    </div>
                    <span
                      className="text-xl font-bold shrink-0"
                      style={{
                        color: prodIntel.worstProduct.avgMargin < 20 ? '#ef4444' : '#f59e0b',
                        textShadow: `0 0 20px ${prodIntel.worstProduct.avgMargin < 20 ? 'rgba(239,68,68,0.5)' : 'rgba(245,158,11,0.5)'}`,
                      }}
                    >
                      {prodIntel.worstProduct.avgMargin.toFixed(0)}%
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          <Panel>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/content" className="text-[#444455] hover:text-[#f0f0f5] transition-colors">
              ...
              <div className="space-y-3.5">
                {topContent.map((c, i) => (
                  <div key={c.id} className="flex items-start gap-2.5">
                    <span
                      className="text-xs font-bold w-5 h-5 rounded-md flex items-center justify-center shrink-0 mt-0.5"
                      style={{
                        background: i === 0 ? 'rgba(167,139,250,0.15)' : 'rgba(255,255,255,0.05)',
                        color: i === 0 ? '#a78bfa' : '#444455',
                      }}
                    >
                      {i + 1}
                    </span>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-[rgba(255,255,255,0.05)] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(globalConversionRate, 100)}%`,
                        background: 'linear-gradient(90deg, #10b981, #34d399)',
                        boxShadow: '0 0 8px rgba(16,185,129,0.5)',
                      }}
                    />
                  </div>
                  <span className="text-[#f0f0f5] text-sm font-bold w-12 text-right shrink-0">
                    {globalConversionRate.toFixed(1)}%
                  </span>
                </div>
                <p className="text-[#444455] text-[11px] mt-1">
                  {wonLeads} de {allLeads.length} leads
                </p>
              </div>
              {bestProject && (
                <div className="pt-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  ...
                  <Link href={`/projects/${bestProject.id}`} className="group">
                    <p className="text-[#f0f0f5] text-sm font-medium group-hover:text-white transition-colors">
                      {bestProject.name}
                    </p>
                    <p className="text-[#10b981] text-xs" style={{ textShadow: '0 0 8px rgba(16,185,129,0.4)' }}>
                      {bestProject.revenue > 0
                        ? `${((bestProject.profit / bestProject.revenue) * 100).toFixed(1)}% margem`
                        : 'Sem receita'}
                    </p>
                  </Link>
                </div>
                  ...
                  return (
                    <div key={p.platform}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[#f0f0f5] text-xs font-semibold tabular-nums">
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ backgroundColor: `${color}1a`, color, boxShadow: `0 0 16px ${color}30` }}
                  >
                    {p.name.charAt(0)}
                  </div>
                  ...
                  {margin !== null && (
                    <p className={`text-xs mt-0.5 font-medium ${margin >= 20 ? 'text-[#10b981]' : 'text-[#f59e0b]'}`}>
