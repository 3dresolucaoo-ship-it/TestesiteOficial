 ...
 *   - LazyMotion + domAnimation carregado via dynamic import.
 *     O runtime de animação (~15KB) não executa no first paint.
 *     Elementos usam `m.*` (subconjunto) em vez de `motion.*` (bundle completo).
 *   - posthog-js removido do chunk Hero (lazy em lib/posthog.ts).
 */
...
// domAnimation carregado via dynamic import — não entra no chunk do Hero.
// Enquanto o feature set não chega: elementos ficam no estado `initial` e animam
// ao receber o chunk. O SSR renderiza HTML limpo (sem opacity:0 inline).
const loadFeatures = () =>
