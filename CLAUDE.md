...
- **Pendência prioritária #1**: **Marca INPI HAYZER** — depositar nominativa nas classes 35 + 42, GRU código 389, lista pré-aprovada. Custo atualizado pós set/2025 (taxa unificada): **R$ 440/classe** PF/MEI/ME/EPP = R$ 880 total OU R$ 880/classe PJ = R$ 1.760 total. Cobre depósito + concessão + 10 anos de proteção. Fluxo: cadastro e-INPI → GRU em `meu.inpi.gov.br/pag/` → pagar → depositar em `marcas.inpi.gov.br/emarcas/`. CEO escolhe: PF (Gabriel Vaz) ou MEI. Posso guiar via Chrome MCP.
