...
| `20260518_notifications_and_search.sql` | **Sino + lupa do dashboard V4 (Bruna 18/05 noite, APLICADA 19/05 madrugada).** Tabela `notifications` (uuid PK, user_id, project_id text, type CHECK 7 eventos, title/body/link, read/read_at). RLS: SELECT/UPDATE/DELETE own user, INSERT bloqueado pra authenticated (service_role bypassa). 2 indices (parcial WHERE read=false + composto user+project). View materializada `search_index` (orders + customers + products com searchable_text em portugues). Indice GIN tsvector + UNIQUE (type,id,project_id) para REFRESH CONCURRENTLY. Funcao `refresh_search_index()` SECURITY DEFINER. **+REVOKE/GRANT extras (Bruna recomendou defesa em profundidade)**: `REVOKE ALL ON search_index FROM anon, authenticated; GRANT SELECT ON search_index TO authenticated`. View materializada não respeita RLS automaticamente, garante anon NUNCA lê + authenticated lê só via services com filtro user_id + project_id. **REFRESH inicial** rodado pra popular a view. Services: `services/notifications.ts` e `services/search.ts`. |
| `20260519_add_vertical_type.sql` | **Multi-vertical schema Onda 1 (Bruna 19/05 -- AGUARDA CEO APLICAR).** Cria ENUM `public.vertical_type ('maker', 'beauty')` + coluna `projects.vertical_type NOT NULL DEFAULT 'maker'` + index `projects_vertical_type_idx`. Nenhuma RLS alterada (isolamento por usuario ja coberto pelas policies existentes; filtro por vertical e responsabilidade do service layer na Onda 2). Onda 1 ate 04/07/2026 = somente maker. Onda 2 a partir de 05/07/2026 = beauty habilitado. ADR: `decisions/016-multi-vertical-schema.md`. |
| ~~`20260520_calc_pro_subscriptions.sql`~~ | **DELETADA 2026-05-21 — Calc Pro freemium descontinuado pelo CEO.** Migration NUNCA foi aplicada em prod (aguardava Stripe setup). ADR-023 revogado. Arquivo SQL removido do repo pra evitar aplicacao acidental. |
| `20260520_leads_converted_order.sql` | **Etapa 3 Golden Path #1 — lead→pedido manual.** Adiciona `leads.converted_order_id text REFERENCES orders(id) ON DELETE SET NULL` + `orders.source_lead_id text REFERENCES leads(id) ON DELETE SET NULL`. 2 índices (leads_converted_order_id_idx, orders_source_lead_id_idx). RLS existente (leads_own / orders_own FOR ALL) cobre automaticamente — sem nova policy. **AGUARDA APLICACAO** (branch feature/etapa3-lead-to-order, nao mergeado em prod). |
| `20260520_user_settings_onboarding.sql` | **Onboarding wizard Bloco 2 (felipe-frontend 29/05).** Cria `user_settings` (user_id uuid PK REFERENCES auth.users, onboarding_completed bool DEFAULT false, onboarding_step smallint DEFAULT 0, onboarding_skipped_at timestamptz, settings jsonb DEFAULT '{}', updated_at timestamptz). RLS habilitada com policy `user_settings_own` (user lê/escreve só própria linha). Trigger `updated_at`. **JÁ APLICADA EM PROD** (verificado via Supabase MCP em 29/05 — tabela existe vazia, RLS ativa). Service: `services/onboarding.ts` (get/saveStep/complete/skip/reset). Hook: `lib/hooks/useOnboarding.ts`. Wizard 4 steps em `components/onboarding/OnboardingWizard.tsx`. Persistência fire-and-forget (graceful fallback se tabela ausente — wizard apenas não aparece, app não quebra). |
...
- ✅ `20260518_notifications_and_search.sql` **APLICADA em prod em 2026-05-19 madrugada** via Supabase MCP `apply_migration`. Inclui REVOKE/GRANT extras pra defesa em profundidade na view materializada (anon REVOKE, authenticated GRANT SELECT) + REFRESH inicial pra popular search_index. Tabela `notifications` + view `search_index` + função `refresh_search_index()` em prod. Próximo passo: configurar pg_cron pra refresh diário (job separado, opcional). Felipe implementando sino + lupa no header V4 nesta sessão.
- ⏳ `20260519_add_vertical_type.sql` **CRIADA em 2026-05-19, AGUARDA CEO APLICAR.** ENUM `public.vertical_type ('maker', 'beauty')` + coluna `projects.vertical_type NOT NULL DEFAULT 'maker'` + index. Nao altera RLS. ADR: `decisions/016-multi-vertical-schema.md`.
