...
| `20260520_calc_pro_subscriptions.sql` | **Calc Pro Subscription (Paulo 20/05 -- AGUARDA CEO STRIPE SETUP + APLICAR).** Cria `calc_pro_subscriptions` (id, user_id, email citext, stripe_customer_id, stripe_subscription_id, stripe_price_id, status CHECK 8 valores, current_period_*, trial_end, cancel_at_period_end, canceled_at, last_event_id/at, timestamps). UNIQUE em stripe_subscription_id + UNIQUE em user_id (1 sub por user). 4 indexes (user_id, status parcial active+trialing, email, customer_id). Trigger updated_at com search_path imutavel. RLS: SELECT own user (auth.uid = user_id); INSERT/UPDATE/DELETE deny-all (so service_role via webhook). RPC `upsert_calc_pro_subscription` SECURITY DEFINER pra upsert idempotente por (stripe_subscription_id). Substitui modelo lifetime R$ 37 — `calculadora_pro_purchases` mantida pra grandfathering. ADR: `decisions/023-calc-pro-freemium-subscription.md`. |
