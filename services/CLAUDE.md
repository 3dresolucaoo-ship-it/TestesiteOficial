...
| calcProSubscription.ts | ~220 | calc_pro_subscriptions | ✅ Paulo 20/05 — service da Calc Pro Subscription (ADR-023). `getSubscriptionByUserId/Email/CustomerId`, `isActiveSubscription` (atalho UI), `upsertSubscription` via RPC `upsert_calc_pro_subscription`. RLS browser pra leitura do user logado; admin pra escrita webhook. Migration `20260520_calc_pro_subscriptions.sql` (NAO aplicada — aguarda CEO Stripe setup). |
