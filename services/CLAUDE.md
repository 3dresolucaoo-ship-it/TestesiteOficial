...
| customersService.ts | ~230 | customers + orders (join) | ✅ Bruna 21/05 — CRUD completo + `getCustomersWithLTV(projectId, userId)` (2 queries, agrega métricas em memória) + `getCustomerSummary(projectId, userId, customerId)` (perfil completo com timeline). Depende da tabela `customers` (migration `20260510_customers_table.sql`). LTV via `customer_id` FK — pedidos legados (customer_id NULL) ficam de fora até Wave 1 dedup. |
